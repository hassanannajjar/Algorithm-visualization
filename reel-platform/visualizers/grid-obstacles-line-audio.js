/**
 * SFX for Unique Paths II (obstacles) grid reel.
 */
(function(global) {
  'use strict';

  var STORAGE_KEY = 'up2LineReelSoundEnabled';
  var ctx = null;
  var masterGain = null;
  var enabled = true;
  var lastTs = 0;
  ...
  function step(lineIdx, blocked) {
    if (!enabled) return;
    if (blocked) { blip(180, 0.06, 0.07, 'sawtooth'); return; }
    if (lineIdx < 0) { blip(240, 0.035, 0.05, 'triangle'); return; }
    if (lineIdx <= 2) { blip(360 + lineIdx * 70, 0.045, 0.08, 'sine'); return; }
    if (lineIdx <= 6) { blip(560 + (lineIdx - 3) * 40, 0.042, 0.075, 'sine'); return; }
  ...
  function playComplete() {
    ...
    [392, 494, 587, 659].forEach(function(hz, i) {
      try {
