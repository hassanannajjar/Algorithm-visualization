/**
 * Procedural SFX for Min Cost Climbing Stairs line-by-line reel (Web Audio API).
 */
(function(global) {
  'use strict';

  var STORAGE_KEY = 'mcsLineReelSoundEnabled';
  ...
  /** @param {number} lineIdx -1 intro, 0–5 code lines */
  function step(lineIdx) {
    ...
    if (lineIdx === 0) {
      blip(320, 0.045, 0.08, 'sine');
      return;
    }
    if (lineIdx === 1) {
      blip(420, 0.05, 0.09, 'sine');
      return;
    }
    if (lineIdx === 2) {
      blip(520, 0.048, 0.085, 'sine');
      return;
    }
    if (lineIdx <= 4) {
      blip(600 + (lineIdx - 3) * 55, 0.042, 0.08, 'sine');
      return;
    }
    blip(820, 0.055, 0.09, 'sine');
  }
  ...
  function playComplete() {
    ...
    [330, 415, 494, 622].forEach(function(hz, i) {
      try {
