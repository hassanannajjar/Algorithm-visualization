/**
 * Procedural SFX for House Robber line-by-line reel (Web Audio API).
 */
(function(global) {
  'use strict';

  var STORAGE_KEY = 'hrLineReelSoundEnabled';
  var ctx = null;
  var masterGain = null;
  var enabled = true;
  var lastTs = 0;
  ...
  function step(lineIdx, decision) {
    if (!enabled) return;
    if (lineIdx < 0) { blip(220, 0.035, 0.05, 'triangle'); return; }
    if (decision === 'rob') { blip(520, 0.05, 0.09, 'square'); return; }
    if (decision === 'skip') { blip(400, 0.042, 0.07, 'triangle'); return; }
