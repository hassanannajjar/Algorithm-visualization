/**
 * SFX for Longest Palindromic Subsequence reel.
 */
(function(global) {
  'use strict';

  var STORAGE_KEY = 'lpsLineReelSoundEnabled';
  var ctx = null;
  var masterGain = null;
  var enabled = true;
  var lastTs = 0;
  ...
  function step(lineIdx, matched) {
    ...
    if (matched) { blip(660, 0.05, 0.09, 'sine'); return; }
    if (lineIdx <= 2) { blip(380 + lineIdx * 60, 0.045, 0.08, 'sine'); return; }
    if (lineIdx <= 5) { blip(520 + (lineIdx - 3) * 45, 0.042, 0.075, 'sine'); return; }
    blip(800, 0.055, 0.09, 'sine');
  }
  ...
  function playComplete() {
    ...
    [440, 554, 659, 880].forEach(function(hz, i) {
      try {
